...
class FirestoreClass {
	...
	var fireStoreInstance =
		fireStore.collection("users").document(currentUser()).collection("movies")
	...
	fun deleteItem(title: String){
		val moviesRef = fireStore.collection("users").document(currentUser()).collection("movies").document(title).delete()
	}
